...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::post('/dashboard/{user}', function (User $user, Request $request) {
        ...
        if ($request->hasFile('photo')) {
            ...
            $user->update([
                'image_profile' => $image->storeAs('images', $imageName, 'public'),
            ]);
        }
    })->name('update-profile');
    ...
    Route::patch('/groups/{group}', [GroupController::class, 'update'])->can('update', 'group')->name('groups.update');
...
Route::post('/tags/{user}/detach/{tag}', [TagController::class, 'removeTagUser'])
